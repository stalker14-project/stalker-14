...
public sealed class PdaMessengerSystem : EntitySystem
{
    ...
    private readonly List<PdaChat> _chats = new() { new PdaChat("Загальний") };
    ...
    private void SendMessage(Entity<PdaMessengerComponent> messenger, ref CartridgeMessageEvent args)
    {
        ...
        if (message.Message.Receiver == "Загальний")
        {
        ...
        while (query.MoveNext(out var uid, out var pda))
        {
            ...
            var newChat = new PdaChat($"Відправник {senderPda.OwnerName}", message.Message.Receiver, senderPda.OwnerName);
