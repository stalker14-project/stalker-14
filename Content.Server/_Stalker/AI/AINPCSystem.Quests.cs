...
namespace Content.Server._Stalker.AI
{
    public sealed partial class AINPCSystem : SharedAiNpcSystem
    {
        ...
        // --- NEW TOOL: Complete Quest ---

        private string GetCompleteQuestToolDescription(AiNpcComponent component)
        {
            // Build the list of possible reward items for the description
            var rewardItemsList = component.GivableItems // Assuming rewards come from GivableItems
        ...
        public bool TryCompleteQuest(EntityUid npc, AiNpcComponent aiComp, string targetPlayerIdentifier, string questItemId, string rewardItemId, int rewardQuantity, string? npcResponse = null)
        {
            ...
            // --- 1. Find Target Player (Needed for both Take and Give) ---
            EntityUid? targetPlayer = FindPlayerByIdentifier(targetPlayerIdentifier);
            if (targetPlayer == null || !targetPlayer.Value.Valid)
            {
                _sawmill.Warning($"Could not find target player '{targetPlayerIdentifier}' for TryCompleteQuest.");
                // TryChat(npc, $"Who are you talking about?"); // Maybe feedback?
                return false;
            }

            // --- 2. Attempt to Take the Quest Item ---
            ...
            if (!TryTakeItem(npc, targetPlayerIdentifier, questItemId, null))
            {
                ...
                // TryChat(npc, $"You don't seem to have the {questItemId} I asked for. Hold it out."); // Feedback
                return false; // Stop if we couldn't take the item
            }

            _sawmill.Info($"NPC {ToPrettyString(npc)} successfully took quest item '{questItemId}' from {ToPrettyString(targetPlayer.Value)}.");

            // --- 3. Attempt to Give the Reward Item ---
            // Ensure quantity is valid
            rewardQuantity = Math.Max(1, rewardQuantity);

            // Pass null for npcResponse for the same reason as above.
            if (!TryGiveItem(npc, targetPlayerIdentifier, rewardItemId, rewardQuantity, null))
            {
                ...
                // TryChat(npc, $"I took the {questItemId}, but something went wrong giving you the {rewardItemId}. Sorry!"); // Feedback
                // Even though giving failed, the quest *item* was taken. Should this return true or false?
                // Let's return false as the full transaction didn't complete as intended.
                return false;
            }
            ...
            // --- 4. Quest Completion Logic (Placeholder) ---
            // TODO: Mark the specific quest (e.g., the one for questItemId from this npc for this player) as completed.
            ...
            return true; // Both take and give succeeded
        }
